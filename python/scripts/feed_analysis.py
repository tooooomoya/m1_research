...
def read_metrics_mean_var_series(data_dir, target_prefix, metric_prefix, var_prefix, count_prefix, num_classes=5, max_index=10000):
    class_means = {i: [] for i in range(num_classes)}
    class_sems = {i: [] for i in range(num_classes)}  # SEMに変更
    steps = []
    ...
    for i in range(max_index + 1):
        ...
        try:
            ...
            for c in range(num_classes):
                ...
                n = max(float(row.get(count_key, 1)), 1)  # countが1未満になるのを防ぐ
